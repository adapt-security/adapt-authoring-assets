...
class Assetsmodule extends AbstractApiModule {
  ...
  async ensureLibPermissions () {
    await Promise.all([
      ...
      try {
        fs.access(cmd, fs.constants.X_OK)
        this.log('debug', `confirmed execute permissions for ${cmd}`)
      } catch (e) {
        if (e.code !== 'EACCES') throw e
        try {
          this.log('debug', `execute permissions disallowed ${cmd}`)
          if (!customCmd) await fs.chmod(lib.path, fs.constants.S_IXUSR)
          this.log('info', `execute permissions updated for ${cmd}`)
        } catch (e) {
          throw new Error(`failed to set execute permissions for ${cmd}`)
        }
      }
    }))
